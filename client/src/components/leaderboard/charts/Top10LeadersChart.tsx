...
function CustomYAxisTick({
  ...
  // Extract team abbreviation from the payload value (format: "name|team")
  const [, team] = payload.value.split("|");
...
export function Top10LeadersChart({
  ...
  // Take top 10
  const chartData: ChartDataPoint[] = entries.slice(0, 10).map((entry) => ({
    playerId: entry.playerId,
    name: entry.name,
    team: entry.team ?? "",
    value: entry.primaryValue,
    rank: entry.rank,
    // Composite key for Y-axis
    nameTeam: `${entry.name}|${entry.team ?? ""}`,
  }));
