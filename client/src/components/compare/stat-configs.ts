...
export interface SkaterStatConfig {
  ...
  label: string;
  tooltip: string;
  /** Optional metric key (e.g. "xG/60") for glossary-driven tooltips */
  metric?: string;
...
export const SKATER_STAT_CONFIGS: SkaterStatConfig[] = [
  ...
  },
  {
    ...
    metric: "CF%",
    format: (v) => (v != null ? formatPercent(v, true) : "-"),
    higherIsBetter: true,
    mode: "rate",
  },
  {
...
export interface GoalieStatConfig {
  ...
  label: string;
  tooltip: string;
  /** Optional metric key (e.g. "GAA") for glossary-driven tooltips */
  metric?: string;
